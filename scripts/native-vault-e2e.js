async (page) => {
  ...
  try {
    ...
    if (!status.exists) {
      ...
      await page.getByLabel('Confirm master password', { exact: true }).fill('Phase2-QA-only-master-8431');
    ...
    check('password creation persisted', (await invoke('vault_unlock', { masterPassword: 'Phase2-QA-only-master-8431' })).some(p => p.title === 'PHASE2-QA credential'));
    ...
    check('password edit persisted', (await invoke('vault_unlock', { masterPassword: 'Phase2-QA-only-master-8431' })).some(p => p.title === 'PHASE2-QA edited'));
